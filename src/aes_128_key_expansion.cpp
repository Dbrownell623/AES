...
void add_round_constant(byte *byte_word, int round_number) {
    byte_word[0] = byte_word[0] ^ Rcon[round_number];
}
...
void g_function(byte *byte_word, int round_number)
{
    ...
    //s-box
    byte_word[0] = sbox[byte_word[0]];
    byte_word[1] = sbox[byte_word[1]];
    byte_word[2] = sbox[byte_word[2]];
    byte_word[3] = sbox[byte_word[3]];
    //add constant
...
void get_round_keys(byte* key, byte* ExpandedKeys) {
    ...
    {
        for (int i = 0; i < 4; i++){
            tmp[i] = ExpandedKeys[i + totalBytes - 4];
        }

        if (totalBytes % 16 == 0)
        {
            g_function(tmp, rconcount);
            rconcount++;     
        }
        for (unsigned char i = 0; i < 4; i++)
        {
            ExpandedKeys[totalBytes] = ExpandedKeys[totalBytes - 16] ^ tmp[i];
            totalBytes++;
        }
    }
}
